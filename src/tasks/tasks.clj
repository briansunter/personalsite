...
(defn- create-pod [deps] (future (create-pod' deps)))

(def ^:private ^:deps toml-metadata-deps
  '[[org.clojure/tools.namespace "0.3.0-alpha3"] [toml "0.1.2"]])

(def ^:private +toml-metadata-defaults+
  {:filterer identity, :extensions [".md" ".adoc" ".org"]})
...
(deftask
  ...
  [_ filterer FILTER code
   "predicate to use for selecting entries (default: `identity`)" e extensions
   EXTENSIONS [str]
   "extensions of files to include (default: `[]`, aka, all extensions)" r
   keep-toml bool "if `true`, remove the yaml header from files"]
...
(deftask photoswipe-album
         ...
         [_ filterer FILTER code
          "predicate to use for selecting entries (default: `identity`)" e
          extensions EXTENSIONS [str]
          "extensions of files to include (default: `[]`, aka, all extensions)"]
         (let [pod (create-pod photoswipe-album-deps)
               options (merge +photoswipe-album-defaults+ *opts*)]
           (perun/assortment-task (merge options
