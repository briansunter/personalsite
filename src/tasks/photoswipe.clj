...
(defn group-albums
  ...
  (let [photo-entries (filter (partial has-tag? "photography") entries)]
    (into {} (for [{:keys [parent-path short-filename] :as entry} photo-entries]
               [(str parent-path "photoswipe/" short-filename ".json") {:entry entry :entries entries}]))))

(defn generate-photoswipe-json [{:keys [entry entries]}]
  (perun/report-debug "photoswipe albums" "processing photoswipe json" (:filename entry))
  (let [images (:images entry)
        all-images (filter :height entries)
        images-by-name (group-by-singly :filename entries)]
    (generate-string {:data (images-to-photoswipe images-by-name images)} {:pretty true})))
